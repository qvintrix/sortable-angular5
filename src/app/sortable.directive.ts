...
export class SortableDirective implements AfterContentInit {
	...
	drag(event) {
		...
		const inside = (event.clientX >= divCoords.left && event.clientX <= divCoords.right
			&& event.clientY >= divCoords.top && event.clientY <= divCoords.bottom);
	...
	private setDataAttributesToChildren() {
		const nativeE = this.element.nativeElement;
		for (let i = 0; i < nativeE.childElementCount; i++) {
			const element = nativeE.children;
			element[i].draggable = true;
			element[i].dataset.index = i;


			const firstChild = element[i].firstElementChild;
			if (firstChild.classList.length && firstChild.classList[0] === 'group') {
				for (let y = 0; y < firstChild.firstElementChild.childElementCount; y++) {
					firstChild.firstElementChild.children[y].draggable = true;
					firstChild.firstElementChild.children[y].dataset.index = nativeE.childElementCount + y;
				}
			}
		}
	}
	...
	private savePositions(attribute) {
		...
		for (let i = 0; i < nativeE.childElementCount; i++) {
			const element = nativeE.children;
			element[i].dataset[attribute] = i;

			const firstChild = element[i].firstElementChild;
			if (firstChild.classList.length && firstChild.classList[0] === 'group') {
				// console.log(firstChild.firstElementChild.childElementCount);
				for (let y = 0; y < firstChild.firstElementChild.childElementCount; y++) {
					firstChild.firstElementChild.children[y].dataset[attribute] = nativeE.childElementCount + y;
				}
			}
		}
	}
